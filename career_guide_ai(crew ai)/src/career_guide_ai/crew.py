...
class CareerGuideAi():
    ...
    @agent
    def interest_analyzer(self) -> Agent:
        return Agent(
            config=self.agents_config['interest_analyzer'], # type: ignore[index]
            verbose=True
        )

    @agent
    def career_recommender(self) -> Agent:
        return Agent(
            config=self.agents_config['career_recommender'], # type: ignore[index]
            verbose=True
        )
    @agent
    ...
    @crew
    def crew(self) -> Crew:
        """Creates the CareerGuideAi crew"""
